...
void	PlayerInventory::draw(sf::RenderTexture & sceneTexture)
{
  ...
  int		mult = (sizeTex.x / 4);
  {
	...
	if ((*it) == true)
	  {
	    tmp.setTextureRect(sf::IntRect(all * mult, 0, mult, sizeTex.y));
	    tmp.setPosition(all * mult, 0);
  ...
  {
    sf::Sprite tmp(_inventoryCursorTex);

    tmp.setPosition(_inventoryCursor * mult, 0);
    sceneTexture.draw(tmp);
  }    
}
